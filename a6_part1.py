...
def create_scatter_plot(data):
    ...
    """
    # Create a figure with size (10, 6)
    plt.figure(figsize=(10, 6))
    
    # Create a scatter plot with Hours on x-axis and Scores on y-axis
    #       Use color='purple' and alpha=0.6
    plt.scatter(data["Hours"], data["Scores"], color='purple', alpha=0.6)
    
    # Add x-axis label: 'Hours Studied'
    plt.xlabel("Hours Studied", fontsize=12)
    
    # Add y-axis label: 'Test Score'
    plt.ylabel("Test Score", fontsize=12)
    
    # Add title: 'Student Test Scores vs Hours Studied'
    plt.title("Student Test Scores vs Hours Studied", fontsize=14, fontweight='bold')
    
    # Add a grid with alpha=0.3
    plt.grid(True, alpha=0.3)
    
    # Save the figure as 'scatter_plot.png' with dpi=300
...
def evaluate_model(model, X_test, y_test):
    ...
    print(f"→ Interpretation: Model explains {r2 * 100:.2f}% of score variation")
    
    # TODO: Print all three metrics with clear labels
    print(f"\nMean Squared Error: {mse:.2f}")
    print(f"Root Mean Squared Error: {rmse:.2f}")
    print(f"→ Interpretation: Predictions are off by about {rmse:.2f} points on average")
...
def visualize_results(X_train, y_train, X_test, y_test, predictions, model):
    ...
    plt.scatter(X_test, predictions, color='red', alpha=0.7, marker='x', s=100, label="Predictions")
